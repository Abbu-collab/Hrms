...
export const EmpRegister = async (req, res) => {
  ...
  try {
    const { name, email, phone, department, password, confirm_password, role } =
      req.body;
...
export const EmpLogin = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ success: false, message: "Check your Email & Password" });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Sorry Invalid Request" });
  }
};
        ...
        const { data, error } = await transporter.emails.send({
            ...
            html: `
                ...
                ">
                    ...
                    <p>Hello ${userExist.name || "Employee"},</p>
...
export const getProfile = async (req, res) => {
  try {
    const user = await Employees.findById(req.user.id).select(
